    ...
    >
      {section.h2 && (
        <h2
          ...
          className="text-2xl sm:text-3xl font-normal tracking-tight text-neutral-800"
        >
          {section.h2}
        </h2>
      )}
      ...
      {section.subSections?.map((sub, j) => (
        <div key={`${sectionKeyPrefix}-subsection-${j}`} className="space-y-4 px-3">
          {sub.h3 && (
            <h3
              id={`subsection-h3-${slugify(sub.h3)}`}
              className="text-xl sm:text-2xl font-light text-left  text-neutral-700"
            >
              {sub.h3}
            </h3>
          )}
          <p className="text-base sm:text-lg text-neutral-600 leading-relaxed text-left ">
            {sub.content.trim()}
          </p>
        </div>
      ))}
